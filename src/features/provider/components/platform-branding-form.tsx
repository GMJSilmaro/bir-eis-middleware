...
import {
  ...
  type PlatformBrandingActionState,
} from "@/features/provider/actions/update-platform-branding.action";
import {
...
export interface PlatformBrandingFormProps {
  initial: {
    productName: string;
...
export function PlatformBrandingForm({ initial }: PlatformBrandingFormProps) {
  ...
  const [state, formAction, pending] = useActionState(
    updatePlatformBrandingAction,
    {} as PlatformBrandingActionState,
  );
    ...
    <form action={formAction} className="space-y-3.5">
      ...
      <div className="space-y-1">
        ...
        <Input
          id="productTagline"
          name="productTagline"
          defaultValue={initial.productTagline}
          className={settingsFieldClassName}
          required
          minLength={2}
          disabled={busy}
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-start gap-3.5">
          ...
          >
            {logo ? (
              // eslint-disable-next-line @next/next/no-img-element -- data URLs / arbitrary URLs
              <img
                src={logo}
          ...
          <div className="flex min-w-0 flex-1 flex-col gap-1.5 pt-0.5">
            ...
            <p className="text-xs leading-relaxed text-muted-foreground">
              Shown on the sign-in page when no tenant overlay is active. PNG,
              JPG, WebP, or SVG up to 5 MB.
            </p>
          </div>
        </div>
        <input
